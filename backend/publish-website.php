...
// Set CORS headers
header('Access-Control-Allow-Origin: http://localhost:3000'); // Updated to match frontend origin
...
if (!$input || empty($input['name'])) {
    ...
    error_log('Invalid input: Website name is required');
    ob_end_flush();
    exit;
}
...
function generatePublishHTML($websiteData) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            forms.forEach(form => {
                form.addEventListener('submit', function(e) {
                    e.preventDefault();
                    alert('Thank you for your message! This is a demo - form submission is not active.');
                });
            });
        });
    </script>
...
function publishWebsite($websiteData, $userId, $pdo) {
    // Generate unique filename
    $websiteSlug = generateSlug($websiteData['name']);
    $fileName = $websiteSlug . '_' . time() . '.html';
    $sitesDir = $_SERVER['DOCUMENT_ROOT'] . '/Promptos/sites';
    $filePath = $sitesDir . '/' . $fileName;
    $websiteUrl = 'http://localhost/Promptos/sites/' . $fileName;
    
    // Create sites directory if it doesn't exist
    if (!is_dir($sitesDir)) {
        if (!mkdir($sitesDir, 0755, true)) {
            throw new Exception('Failed to create sites directory');
        }
        chmod($sitesDir, 0755);
    }
    
    // Generate HTML content
    $html = generatePublishHTML($websiteData);
    
    // Save HTML file
    if (!file_put_contents($filePath, $html)) {
        throw new Exception('Failed to save website file: Permission denied or invalid path');
    }
    
    // Save to database
    try {
        $stmt = $pdo->prepare(
            'INSERT INTO websites (user_id, name, type, slug, file_path, website_url, data, created_at, updated_at) 
             VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
             ON DUPLICATE KEY UPDATE 
             name = VALUES(name), type = VALUES(type), data = VALUES(data), updated_at = NOW()'
        );
...
try {
    $websiteUrl = publishWebsite($input, $_SESSION['user_id'], $pdo);
    ...
    error_log('Error in publish-website.php: ' . $e->getMessage());
